...
. ($loadEnvPath)
$TestRecordingFile = Join-Path $PSScriptRoot 'Update-ZNAssetsMfaInboundPolicy.Recording.json'
...
Describe 'Update-ZNAssetsMfaInboundPolicy' {
    It 'UpdateExpanded' {
        ...
        Update-ZNAssetsMfaInboundPolicy -AssetId $asset -ReactivePolicyId $policy.ItemId -Description $newdescription
        $updatedPolicy = Get-ZNAssetsMfaInboundPolicy -AssetId $asset -ReactivePolicyId $policy.ItemId
