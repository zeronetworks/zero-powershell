if(($null -eq $TestName) -or ($TestName -contains 'New-ZNUserMfaInboundPolicy'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'New-ZNUserMfaInboundPolicy.Recording.json'
...
Describe 'New-ZNUserMfaInboundPolicy' {
    It 'CreateExpanded' -skip {
        ...
        Remove-ZNUserMfaInboundPolicy -UserId $user.id -ReactivePolicyId $policy.ItemId
    }
}
