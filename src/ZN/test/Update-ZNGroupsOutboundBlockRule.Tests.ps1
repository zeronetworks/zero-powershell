...
Describe 'Update-ZNGroupsOutboundBlockRule' {
    It 'UpdateExpanded' {
        ...
        $rule = New-ZNOutboundBlockRule -LocalEntityId $group.id -LocalProcessesList @("*") -PortsList $portsList -RemoteEntityIdsList @($destination.id) -State 1 -ExpiresAt $expiresAt
        
        $newdescription = "new description" + (Get-Random -Minimum 1 -Maximum 100)
        Update-ZNGroupsOutboundBlockRule -GroupId $group.id -GroupType tag -RuleId $rule.Item.Id -Description $newdescription
        $updatedRule = Get-ZNGroupsOutboundBlockRule -GroupId $group.id -GroupType tag -RuleId $rule.Item.Id
