...
Describe 'Get-ZNGroupMemberOf' {
    It 'Get' {
        $group = (Get-ZNGroup -Search "Domain Admins").Items | Select-Object -first 1
        (Get-ZNGroupMemberOf -GroupId $group.id -grouptype ad).Items.Count | Should -BeGreaterThan 0
    }
}
