...
Describe 'New-ZNAssetInboundOtRule' {
    It 'CreateExpanded' {
        ...
        $rule = New-ZNAssetInboundOtRule -AssetId $asset -Action 1 -Direction 1 -localEntityId $destination.Id -RemoteEntitiesIdList @($asset) -protocolsList $protocolsList -state 1 -LocalProcessesList @("*") -ExcludedLocalIdsList @() -ShouldBuildMirrorRules -ExpiresAT $expiresAt
